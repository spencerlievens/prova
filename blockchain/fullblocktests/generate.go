...
func payToScriptHashScript(redeemScript []byte) []byte {
	redeemScriptHash := rmgutil.Hash160(redeemScript)
...
func (g *testGenerator) createCoinbaseTx(blockHeight int32) *wire.MsgTx {
	...
	uniquenessScript, err := scriptBuilder.AddOp(txscript.OP_RETURN).AddInt64(int64(blockHeight)).Script()
	if err != nil {
		panic(err)
	}
	tx.AddTxOut(&wire.TxOut{
...
func additionalCoinbase(amount rmgutil.Amount) func(*wire.MsgBlock) {
...
func additionalSpendFee(fee rmgutil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
...
func createSpendTxForTx(tx *wire.MsgTx, fee rmgutil.Amount) *wire.MsgTx {
...
func Generate(includeLargeReorg bool) (tests [][]TestInstance, err error) {
	...
	// Provision an ISSUING key in b1 and check its there.
	// value in parens is which output is spent:
	//
	//   ... -> b1(0) -> b2(1)

	pubKey, _ := btcec.ParsePubKey([]byte{0x02, 0x5c, 0xee, 0xba, 0x2a, 0xb4, 0xa6, 0x35, 0xdf, 0x2c, 0x03, 0x01, 0xa3, 0xd7, 0x73, 0xda, 0x06, 0xac, 0x5a, 0x18, 0xa7, 0xc3, 0xe0, 0xd0, 0x9a, 0x79, 0x5d, 0x7e, 0x57, 0xd2, 0x33, 0xed, 0xf1}, btcec.S256())
	//pubKeyBytes, _ := hex.DecodeString("025ceeba2ab4a635df2c0301a3d773da06ac5a18a7c3e0d09a795d7e57d233edf1")
	adminKeyAddTx := createAdminTx(outs[0], 1, txscript.OP_ISSUINGKEYADD, pubKey)
	g.nextBlock("b1", nil, additionalTx(adminKeyAddTx))

	acceptedWithAdminKey(pubKey)

	// Revoke the ISSUING key in b2 and check its gone.
	...
	// The reorg should revent the ISSUING key revocation in b2.
	//
	//   ... -> b1(0) -> b2(1)
	//               \-> b3(1) -> b4(2)
