...
func dbIndexConnectBlock(dbTx database.Tx, indexer Indexer, block *rmgutil.Block, view *blockchain.UtxoViewpoint) error {
...
func dbIndexDisconnectBlock(dbTx database.Tx, indexer Indexer, block *rmgutil.Block, view *blockchain.UtxoViewpoint) error {
...
func makeUtxoView(dbTx database.Tx, block *rmgutil.Block) (*blockchain.UtxoViewpoint, error) {
	...
	for txIdx, tx := range block.Transactions() {
		...
		for _, txIn := range tx.MsgTx().TxIn {
			...
			view.AddTxOuts(rmgutil.NewTx(originTx), 0)
		}
	}
...
//
// This is part of the blockchain.IndexManager interface.
func (m *Manager) ConnectBlock(dbTx database.Tx, block *rmgutil.Block, view *blockchain.UtxoViewpoint) error {
...
//
// This is part of the blockchain.IndexManager interface.
func (m *Manager) DisconnectBlock(dbTx database.Tx, block *rmgutil.Block, view *blockchain.UtxoViewpoint) error {
	// Call each of the currently active optional indexes with the block
