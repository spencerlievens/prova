...
// newAddressPubKey returns a new rmgutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) rmgutil.Address {
	addr, err := rmgutil.NewAddressPubKey(serializedPubKey,
...
// newAddressPubKeyHash returns a new rmgutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) rmgutil.Address {
	addr, err := rmgutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams)
	if err != nil {
...
// newAddressScriptHash returns a new rmgutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) rmgutil.Address {
	addr, err := rmgutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := rmgutil.NewAddressPubKeyHash(decodeHex("e34cce70c863"+
	...
	p2shMain, _ := rmgutil.NewAddressScriptHashFromHash(decodeHex("e8c300"+
	...
	p2pkCompressedMain, err := rmgutil.NewAddressPubKey(decodeHex("02192d74"+
	...
	p2pkUncompressedMain, err := rmgutil.NewAddressPubKey(decodeHex("0411db"+
	...
	tests := []struct {
		...
		{(*rmgutil.AddressPubKeyHash)(nil), "", txscript.ErrUnsupportedAddress},
		{(*rmgutil.AddressScriptHash)(nil), "", txscript.ErrUnsupportedAddress},
		{(*rmgutil.AddressPubKey)(nil), "", txscript.ErrUnsupportedAddress},
