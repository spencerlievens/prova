...
func (cs *CoinSet) TotalValue() (value rmgutil.Amount) {
...
func satisfiesTargetValue(targetValue, minChange, totalValue rmgutil.Amount) bool {
...
type CoinSelector interface {
	CoinSelect(targetValue rmgutil.Amount, coins []Coin) (Coins, error)
}
...
type MinIndexCoinSelector struct {
	MaxInputs       int
	MinChangeAmount rmgutil.Amount
}

// CoinSelect will attempt to select coins using the algorithm described
// in the MinIndexCoinSelector struct.
func (s MinIndexCoinSelector) CoinSelect(targetValue rmgutil.Amount, coins []Coin) (Coins, error) {
	cs := NewCoinSet(nil)
...
func (s MinNumberCoinSelector) CoinSelect(targetValue rmgutil.Amount, coins []Coin) (Coins, error) {
...
func (s MaxValueAgeCoinSelector) CoinSelect(targetValue rmgutil.Amount, coins []Coin) (Coins, error) {
	sortedCoins := make([]Coin, 0, len(coins))
	sortedCoins = append(sortedCoins, coins...)
...
func (s MinPriorityCoinSelector) CoinSelect(targetValue rmgutil.Amount, coins []Coin) (Coins, error) {
...
// rmgutil.Tx, a specific outpoint index, and the number of confirmations
...
func (c *SimpleCoin) Value() rmgutil.Amount {
	return rmgutil.Amount(c.txOut().Value)
}
