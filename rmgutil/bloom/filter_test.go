...
func TestFilterInsertKey(t *testing.T) {
	...
	wif, err := rmgutil.DecodeWIF(secret)
	if err != nil {
	...
	f.Add(rmgutil.Hash160(wif.SerializePubKey()))
...
func TestFilterBloomMatch(t *testing.T) {
	...
	tx, err := rmgutil.NewTxFromBytes(strBytes)
	if err != nil {
	...
	spendingTx, err := rmgutil.NewTxFromBytes(spendingTxBytes)
	if err != nil {
		t.Errorf("TestFilterBloomMatch NewTxFromBytes failure: %v", err)
		return
	}
